...
impl Material {
    ...
    pub fn get_color(&self, part: &MaterialPart) -> Option<(f32, f32, f32)> {
        match part {
            MaterialPart::Color(r, g, b) => Some((*r, *g, *b)),
            _ => None
        }
    }

    // pub fn get_diffuse_texture<'a>(&'a self, registry: &'a Registry) -> Option<&'a Texture> {
    //     match self.diffuse {
    //         MaterialPart::Texture(ambient) => match registry.get_resource(&ambient) {
    //             Some(texture) => Some(texture),
    //             None => None
    //         },
    //         _ => None
    ...
    // }
    //
    // pub fn get_specular_texture<'a>(&'a self, registry: &'a Registry) -> Option<&'a Texture> {
    //     match self.specular {
    //         MaterialPart::Texture(ambient) => match registry.get_resource(&ambient) {
    //             Some(texture) => Some(texture),
    //             None => None
    ...
    pub fn chrome() -> Self {
        Material {
            ambient: MaterialPart::Color(0.25, 0.25, 0.25),
            diffuse: MaterialPart::Color(0.4, 0.4, 0.4),
            specular: MaterialPart::Color(0.774597, 0.774597, 0.774597),
            shininess: 0.6 * 128.0
        }
    }
}
